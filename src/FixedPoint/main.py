...
# ===================================================================================================================
def normalize_weight_model(train_loader, valid_loader, test_loader, device, args, round_num):
    # Initialize model
    print(f'Model Initialization in Round {round_num} ...')
    model = resnet20.ResNet20(num_classes=args.num_of_class).to(device)
    # Define loss function and optimizer
    ...
    print(f'Start Training for normalizing weights between -1 and 1 in Round {round_num} ...\n')
    best_acc = 0
    epc = 0
    for ep in range(args.epochs):
        start_time = time.time()
...
# ===================================================================================================================
#                       Train, Validation, and Test Process for Adding Noise to the System
# ===================================================================================================================
def noisy_train_validation(train_loader, valid_loader, test_loader, device, args, round_num, mode='standard'):
...
def attack(valid_loader, test_loader, device, args, round_num, mode='standard', attack_type='sign'):
    ...
    if mode == 'standard':
        ...
        print(f'Load The The standard model with bit mapping training in Round {round_num}'
              f' For {attack_type} attack')
...
if __name__ == '__main__':
    ...
    for rnd in range(args.round_number):
        ...
        print(f'----------------------------------------------------------------------------------------')
        train_and_validate(train_loader, valid_loader, test_loader, device, args, rnd + args.start_point)

        print(f'----------------------------------------------------------------------------------------')
        print(f'      Noisy Training and Validation On Standard Model in round {rnd + args.start_point} is started ...')
        print(f'----------------------------------------------------------------------------------------')
        noisy_train_validation(train_loader, valid_loader, test_loader, device, args, rnd + args.start_point,
                               mode='standard')
        ...
        attack(valid_loader, test_loader, device, args, rnd + args.start_point, mode='BitMapped', attack_type='multiBit')
